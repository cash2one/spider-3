        ...
        function sliderway(){
            ...
            ricylepic = setInterval(function(){
                ...
                if(jq('.to8to_headgif').eq(nowIndex).next().length > 0){
                    ...
                    // 置scrollTop为0，且过1.5秒开始轮播
                    clearInterval(ricylepic);
                    // 初始化
                    jq('.slider_container').scrollTop(0);
                    firstslider = setTimeout(function(){
                        sliderway();
                    },500);    // 最后一张图片多停留500ms
                }
            },1530);
...
function city_from_ip(){
    var sCity = '';
    if (getCookie('city_from_ip').length > 0) {//cookie存在值就先读取cookie
        sCity = getCookie('city_from_ip');//获取cookie的值
    }else{
        var ipCity = '';            
        var num = Math.floor((Math.random()*10000000000+1)).toString();
        //根据ip获取城市接口
        jq.ajax({
            url: 'http://www.to8to.com/api/getAreaInfo.php',
            data: 'act=getIp&_='+num,
            type: "GET",
            async: false,//改成同步
            dataType: 'jsonp',
            success: function(data) {
                ipCity = data.data.to8to_tname;
                if (ipCity.length != 0) {//获取到的字符长度不为0
                    sCity = ipCity;
                }else {
                    sCity = '北京';                     
                }
                setCookie('city_from_ip',sCity,365*24*60*60*1000);//如果接口有返回城市信息 则用cookie保存
            }
        }); 
    }    
    return sCity;//返回城市信息
