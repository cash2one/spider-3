jq(function () {
    //导航 版本测试
    (function () {
        var b = jq('body,html'), n = jq('.about_nav'), bg = jq('.about_nav_bg'), j = jq('#jlbz_dh'), top = n.offset().top;
        if (n.find("li").eq(3).hasClass("about_nav_cur")) return;
        b.stop().animate({ scrollTop: 129 }, 500, function () {
            var a = true;
            jq(window).bind('scroll', function () {
                ...
                /*if (bg.length) {
                    if (t > 129 && a) {
                        navigator.userAgent.indexOf('Firefox') > 0 ? b.animate({ scrollTop: 211 }) : b.scrollTop(211);
                        a = false;
                    }
                    else if (t == 0 && !a) {
                        a = true;
...
jq(function () {
    ...
    (function () {
        ...
        function tab(obj) {
			var mrgnPx = parseInt(jq('.people_talk_list').css('margin-left')),
				offsetNum = mrgnPx/146 || 0,
				idx = obj[0].index - offsetNum;

			for (var i = 0; i < len; i++) {
				if (aLi[i].offsetLeft == arrPos[2 - offsetNum]) {
					aLi.eq(i).animate({ 'left': aLi[obj[0].index].offsetLeft + 'px' });
                }
            }
            try {
                aLi.eq(obj[0].index).css({ 'z-index': zIndex++ });
                aLi.eq(obj[0].index).animate({ 'left': arrPos[2 - offsetNum] + 'px' }, function () {
        ...
        if (location.href.match('about/index')) {
            //初始
            tab(aImg.eq(3));
        }

		//add by windy 2015-4-24
		var leftRow = jq('.chgLeft');
		var rightRow = jq('.chgRight');

		leftRow.click(function() {
			var mrgnPx = jq('.people_talk_list').css('margin-left').replace('px',''),
				offsetNum = Math.abs(mrgnPx/146 || 0);

			if(offsetNum > 0 && !ulObj.is(':animated')) {
				ulObj.animate({'margin-left': '+='+146+'px'}, 500);
			}
		});
		rightRow.click(function() {
			...
			if(offsetNum < (len - 5) && !ulObj.is(':animated')) {
				ulObj.animate({'margin-left': '-='+146+'px'}, 500);
			}
		});
		//End add
                    ...
                    if (point) {
                        if (obj.state == undefined) {
                            ...
                            var HTML = "<div id=" + tag_id + " class=\"" + tag_class + "\">"
                                    ...
                                    + "<p>地址：" + sAddress + "</p>"
                                    + "<a href=" + jq(obj).find('.c_url').text().replace(/[\u4e00-\u9fa5]/g, "") + " target=\"_blank\" class=\"look_this_website\">查看网站</a>"
